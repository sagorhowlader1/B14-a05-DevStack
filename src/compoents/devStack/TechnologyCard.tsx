...
const TechnologyCard = ({ technology }) => {
    ...
    const [addToStack, setAddedToStack] = useState<boolean>(false);

    const handleAddToStack = () => {
        if(addedToStack){
            setAddedToStack(false);
        }else{
...
//   console.log(technology);
  return (
    <div className="grid grid-cols-3 ">
      {technology.map((technology:IDevStack, index:number) => {
        return (
          <div className="card bg-base-600 rounded-2xl my-8  mx-8 shadow-sm py-3 pb-6">
            <div className="flex flex-2 gap-7 justify-between px-6 py-5">
              ...
              <div className="card-actions justify-center">
                <button onClick={() => handleAddToStack()} className="btn bg-black text-[#FFFFFF] font-medium px-40">Add to Stack</button>
              </div>
          </div>
        );
      })}
